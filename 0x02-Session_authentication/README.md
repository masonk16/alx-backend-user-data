...
Now you an “in-memory” Session ID storing. You will be able to retrieve a `User` id based on a Session ID.
...
* Retrieve the `User` instance based on the `email` - you must use the class method `search` of `User` (same as the one used for the `BasicAuth`)
  ...
  * Otherwise, create a Session ID for the `User` ID:
    ...
    * Return the dictionary representation of the `Use`r - you must use `to_json()` method from User
